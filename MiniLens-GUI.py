...
def _center_widget(widget):
    """Envuelve un widget en un contenedor que lo centra horizontalmente.

    Los setItemWidget del arbol estiran el widget a toda la celda; con este
    wrapper el boton queda centrado como en las tablas viejas.
    """
    holder = QWidget()
...
class KubeconfigViewerWindow(QMainWindow):
    def __init__(self):
        ...
        self.pf_manager.updated.connect(self._on_pf_updated)
        self._import_log = []
        ...
        self.lbl_pods_list_title = QLabel(f"{ICON_POD}  Recursos")
        self.lbl_pods_list_title.setObjectName("detail_value")
        list_layout.addWidget(self.lbl_pods_list_title)

        # ARBOL UNICO: cada service es una fila expandible y adentro sus pods
        # (se vinculan por el selector del service contra los labels del pod):
        self.tree_resources = QTreeWidget()
        self.tree_resources = QTreeWidget()
        self.tree_resources.setColumnCount(8)
        self.tree_resources.setHeaderLabels(["#", "NAME", "STATUS / TYPE", "READY", "IP", "PORTS", "AGE", "LOGS"])
        self.tree_resources.header().setSectionResizeMode(0, QHeaderView.Fixed)
        self.tree_resources.setColumnWidth(0, 44)
        self.tree_resources.header().setSectionResizeMode(1, QHeaderView.Stretch)
        self.tree_resources.header().setSectionResizeMode(2, QHeaderView.ResizeToContents)
        self.tree_resources.header().setSectionResizeMode(3, QHeaderView.ResizeToContents)
        # IP CON ANCHO FIJO: si es ResizeToContents el boton amarillo colapsa:
        self.tree_resources.header().setSectionResizeMode(4, QHeaderView.Fixed)
        self.tree_resources.setColumnWidth(4, 130)
        self.tree_resources.header().setSectionResizeMode(5, QHeaderView.Fixed)
        self.tree_resources.setColumnWidth(5, 170)
        self.tree_resources.header().setSectionResizeMode(6, QHeaderView.ResizeToContents)
        self.tree_resources.header().setSectionResizeMode(7, QHeaderView.Fixed)
        self.tree_resources.setColumnWidth(7, 90)
        # SIN STRETCH EN LA ULTIMA COLUMNA: si no, LOGS (vacia) se come el
        # espacio y la columna NAME queda aplastada:
        self.tree_resources.header().setStretchLastSection(False)
        self.tree_resources.setUniformRowHeights(True)
        self.tree_resources.itemDoubleClicked.connect(self.on_tree_double_click)
        list_layout.addWidget(self.tree_resources, stretch=1)

        self.resource_tabs.addTab(list_tab, f"{ICON_DETAIL}  Lista")

        # BARRA DE BUSQUEDA: como widget de esquina del QTabWidget, queda
        # a la altura de las pestañas Lista/Mapa y bien a la derecha,
        # sin ocupar una fila extra abajo:
        self.txt_search = QLineEdit()
        self.txt_search.setPlaceholderText("Buscar pod o service...")
        self.txt_search.setClearButtonEnabled(True)
        self.txt_search.setMaximumWidth(260)
        self.txt_search.setStyleSheet(
            "QLineEdit { background-color: #232323; border: 1px solid #3a3a3a;"
            " border-radius: 4px; padding: 3px 8px; font-size: 13px; }"
            "QLineEdit:focus { border: 1px solid #f5c518; }"
        )
        self.txt_search.textChanged.connect(self._filter_resource_tree)
        self.resource_tabs.setCornerWidget(self.txt_search, Qt.TopRightCorner)
    ...
    def on_clear_config(self):
        ...
        self.lbl_pods_list_title.setText(f"{ICON_POD}  Recursos")
    ...
    def on_open_port_forwards(self):
        ...
        if getattr(self, "_pf_panel", None) is None:
            self._pf_panel = PortForwardPanel(self.pf_manager, self, new_forward_cb=self._pf_new_forward)
        self._pf_panel.show()
        self._pf_panel.raise_()
        self._pf_panel.activateWindow()

    def _pf_new_forward(self):
        # 1 - DIALOGO PARA FORWARDEAR UN PUERTO NUEVO (elijo pod + puerto):
        pods = list(getattr(self, "_pods_cache", {}).values())
        if not pods:
            QMessageBox.information(
                self,
                f"{ICON_INFO} Sin pods",
                "No hay pods cargados: conectate a un cluster y carga los recursos primero.",
            )
            return
        dlg = PortForwardDialog(sorted(pods, key=lambda p: p.metadata.name), self)
    ...
    def start_port_forward(self, pod, pod_port, local_port=None, https=False, open_browser=False):
        ...
        return self.pf_manager.start(
            pod_name=pod.metadata.name,
            namespace=pod.metadata.namespace or "default",
            pod_port=pod_port,
            local_port=local_port,
            kubeconfig=self._loaded_file_path or "",
            context=self.selected_context_name or "",
            https=https,
            open_browser=open_browser,
        )

    def _identity(self):
        # 1 - ARMO LA IDENTIDAD ACTUAL: user@cluster/context        #     (formato: cyberdefensa-oidc@clavaria-dev/cyber-battle-lab):
    ...
    def _on_resources_done(self, ok, data):
        ...
        # 3 - LLENO EL ARBOL (services como padres, pods adentro):
        self._pods_cache = {pod.metadata.name: pod for pod in pods}
        self._fill_resource_tree(pods, services)

        # 4 - GUARDO CACHE Y RENDERIZO EL MAPA GRAFICO:
        ...
        self.lbl_pods_list_title.setText(
            f"{ICON_POD}  Recursos:  ({n_svcs} services, {n_pods} pods)"
        )
        self.lbl_connection_status.setText(
    ...
    def _make_pod_tree_item(self, pod):
        # 1 - ARMO LA FILA DEL POD PARA EL ARBOL (mismas columnas que la tabla vieja):
        name = pod.metadata.name

        # 1a - ESTADO DEL POD (phase):
        status = pod.status.phase if pod.status and pod.status.phase else "Unknown"

        # 1b - CONTAINERS READY (ej: "1/1"):
        ...
        item.setData(1, Qt.UserRole, {"kind": "pod", "name": name})

        # 1d - CELDAS CENTRADAS (status, ready y age):
        item.setTextAlignment(2, Qt.AlignCenter)
        item.setTextAlignment(3, Qt.AlignCenter)
        item.setTextAlignment(6, Qt.AlignCenter)

        # 1e - STATUS CON COLOR (verde/amarillo/rojo como el mapa):
        color = {
            "Running": "#4caf50",
            "Succeeded": "#4caf50",
            "Pending": "#f5c518",
    ...
    def _attach_pod_widgets(self, item, pod):
        # 1 - WIDGETS DE LA FILA DEL POD (IP + LOGS):
        #     IMPORTANTE: hay que llamarlo DESPUES de insertar el item en el
        #     arbol (addChild/addTopLevelItem); si no, Qt no renderiza nada.
        name = pod.metadata.name
        pod_ip = item.text(3)
    ...
    def _attach_pod_widgets(self, item, pod):
        ...
        # 1a - IP DEL POD como boton amarillo cliqueable (clic = copiar):
        if pod_ip and pod_ip != "-":
            self.tree_resources.setItemWidget(
                item, 4, _center_widget(_make_ip_button(pod_ip, tooltip_prefix="IP del POD"))
            )

        # 1b - BOTON LOGS: abre el popup con los logs en vivo del pod:
        ...
        self.tree_resources.setItemWidget(item, 7, _center_widget(btn_logs))

    def _fill_resource_tree(self, pods, services):
        # 1 - LIMPIO EL ARBOL:
        self.tree_resources.clear()

        # 2 - INDEXO LOS PODS POR SERVICE (selector del service vs labels del pod):
        pods_by_svc = {}
        matched = set()
        for svc in services:
            ...
            if selector:
                for p in pods:
                    labels = (p.metadata.labels or {}) if p.metadata else {}
                    if all(labels.get(k) == v for k, v in selector.items()):
                        hijos.append(p)
                        matched.add(p.metadata.name)
            pods_by_svc[svc.metadata.name] = hijos

        # 3 - UNA FILA POR SERVICE, CON SUS PODS DESPLEGADOS ADENTRO:
        #     (los services no llevan numero; los pods se numeran 1..N
        #     igual que la tabla de pods de antes):
        n_pod = 0
        for svc in services:
            ...
            hijos = pods_by_svc.get(svc_name, [])

            svc_item = QTreeWidgetItem([
                "",
                f"{ICON_CLUSTER}  {svc_name}",
                svc_type,
                f"{len(hijos)} pods" if hijos else "-",
                cluster_ip if cluster_ip else "-",
                "",
                _format_age(created),
                "",
            ])
            # 3a - ESTILO DEL SERVICE: nombre en amarillo y negrita:
            svc_item.setForeground(1, QBrush(QColor("#f5c518")))
            svc_font = QFont()
            svc_font.setBold(True)
            svc_item.setFont(1, svc_font)

            # 3b - CELDAS CENTRADAS (tipo, ready y age):
            for col in (2, 3, 6):
                svc_item.setTextAlignment(col, Qt.AlignCenter)

            # 3c - PRIMERO INSERTO EL ITEM EN EL ARBOL, DESPUES LOS WIDGETS
            #      (si seteo widgets antes de insertar, Qt no los renderiza):
            self.tree_resources.addTopLevelItem(svc_item)
            if cluster_ip and cluster_ip != "-":
                self.tree_resources.setItemWidget(
                    svc_item, 4, _center_widget(_make_ip_button(cluster_ip, tooltip_prefix="IP del SERVICE"))
                )
            # 3c - PUERTOS COMO BADGES AMARILLOS: clic en un badge abre el
            #      popup para dar de alta el port forward de ese puerto:
            w_ports = self._make_ports_widget(svc, hijos)
            if w_ports is not None:
                self.tree_resources.setItemWidget(svc_item, 5, w_ports)
            else:
                svc_item.setText(5, "-")
            for p in hijos:
                n_pod += 1
                child = self._make_pod_tree_item(p)
                child.setText(0, str(n_pod))
                svc_item.addChild(child)
                self._attach_pod_widgets(child, p)

        # 4 - PODS SIN SERVICE: VAN EN UN GRUPO PROPIO AL FINAL:
        sueltos = [p for p in pods if p.metadata.name not in matched]
        if sueltos:
            root = QTreeWidgetItem([
                "",
                f"{ICON_POD}  (sin service)",
                "",
                f"{len(sueltos)} pods",
                "",
                "",
            ...
            root.setForeground(1, QBrush(QColor("#f5c518")))
            root.setTextAlignment(3, Qt.AlignCenter)
            self.tree_resources.addTopLevelItem(root)
            for p in sueltos:
                n_pod += 1
                child = self._make_pod_tree_item(p)
                child.setText(0, str(n_pod))
        ...
        # 5 - EXPANDO LOS SERVICES QUE TIENEN PODS:
        for i in range(self.tree_resources.topLevelItemCount()):
            top = self.tree_resources.topLevelItem(i)
            top.setExpanded(top.childCount() > 0)

    def _make_ports_widget(self, svc, hijos):
        # 1 - BADGES AMARILLOS CON EL NUMERO DE PUERTO (sin el /TCP):
        #     CLIC EN UN BADGE = abre el popup para dar de alta el port
        #     forward de ese puerto del service (sin combobox de puerto):
        ...
        lay.addStretch()
        for p in svc.spec.ports:
            # 1a - RESUELVO EL PUERTO REAL DEL POD (target_port):
            #      - si target_port es numerico y distinto de port, uso target_port
            #      - si target_port es nombrado (ej "http"), lo busco en los
            #        container ports del primer pod que matchee
            #      - sino uso el port del service
            #      (esto es clave: kubectl port-forward pod/x local:PUERTO
            #       forwardea al PUERTO DEL POD, no al del service)
            pod_port = self._resolve_target_port(p, hijos)
            target = f" -> {pod_port}" if pod_port != int(p.port) else ""
            b = QPushButton(str(p.port))
            b.setCursor(Qt.PointingHandCursor)
            b.setToolTip(
                f"Forward del puerto {p.port}{target} (clic para abrir el tunnel)"
            )
            b.setStyleSheet(
                "QPushButton { background-color: #f5c518; color: #1a1a1a;"
    ...
    def _resolve_target_port(self, svc_port, pods):
        # 1 - SI target_port ES NUMERICO, USO ESE (es el puerto del pod):
        tp = getattr(svc_port, "target_port", None)
        if tp is not None:
            try:
                return int(tp)
            except (TypeError, ValueError):
                pass
        # 2 - SI target_port ES NOMBRADO (ej "http"), LO BUSCO EN LOS
        #     CONTAINER PORTS DEL PRIMER POD QUE TENGA ESE NOMBRE:
        if tp:
            name = str(tp)
            for pod in (pods or []):
                for c in (pod.spec.containers or []):
                    for cp in (c.ports or []):
                        if cp.name == name or str(cp.container_port) == name:
    ...
    def _filter_resource_tree(self, text):
        ...
        tree = self.tree_resources
        for i in range(tree.topLevelItemCount()):
            top = tree.topLevelItem(i)
            top_match = q in (top.text(1) or "").lower()
            some_child = False
            for j in range(top.childCount()):
                child = top.child(j)
                child_match = q in (child.text(1) or "").lower()
                child.setHidden(bool(q) and not top_match and not child_match)
                if not child.isHidden():
                    some_child = True
            # el service queda visible si su nombre matchea o si tiene pods visibles:
            top.setHidden(bool(q) and not top_match and not some_child)
            if q:
                top.setExpanded(top_match or some_child)

    def on_tree_double_click(self, item, column):
        # 1 - SOLO LOS PODS TIENEN DETALLE (los services solo expanden/colapsan):
        data = item.data(1, Qt.UserRole) or {}
        if data.get("kind") != "pod":
            return

        # 2 - BUSCO EL POD EN EL CACHE Y ABRO EL DETALLE:
        pod = self._pods_cache.get(data.get("name"))
        if pod:
    ...
    def on_discovery(self):
        ...
        task = _DiscoveryTask(v1, self._discovery_signals, sorted(known_ns))
        self._api_pool.start(task)
    ...
    def _open_pod_detail(self, pod):
        ...
        detail_window = PodDetailWindow(
            pod,
            self,
            context_name=getattr(self, "selected_context_name", None),
            cluster_server=getattr(self, "_selected_cluster_server", None),
            pf_starter=self.start_port_forward,
        )
        detail_window.exec()
...
class PortForwardManager(QObject):
    """Gestor de port forwards: lanza 'kubectl port-forward' como proceso
    (con el mismo kubeconfig + context, asi usa el exec OIDC), vigila que
    este vivo y si se cae lo reconecta automaticamente cada 1 minuto,
    hasta que el usuario apague ese forward."""
    updated = Signal()
    ...
    def start(self, pod_name, namespace, pod_port, local_port=None, kubeconfig="", context="", https=False, open_browser=False):
        # 1 - CREO LA ENTRADA Y LANZO EL TUNEL:
        e = {
            "id": self._next_id,
            "pod_name": pod_name,
            "namespace": namespace,
            "pod_port": int(pod_port),
            "local_port": int(local_port) if local_port else 0,
            "local_port_real": int(local_port) if local_port else 0,
            "https": bool(https),
    ...
    def _spawn(self, e):
        # 1 - LANZO kubectl port-forward (mismo kubeconfig + context que la app):
        args = [
            ...
            "port-forward",
            f"pod/{e['pod_name']}",
            f"{e['local_port'] or 0}:{e['pod_port']}",
        ]
        proc = QProcess(self)
        proc.setProcessChannelMode(QProcess.MergedChannels)
        proc.readyReadStandardOutput.connect(lambda eid=e["id"]: self._on_output(eid))
        proc.finished.connect(lambda code, st, eid=e["id"]: self._on_exit(eid))
        proc.start("kubectl", args)
        e["process"] = proc
        e["state"] = "conectando"
        e["last_attempt"] = time.time()

    def _find(self, eid):
        return next((x for x in self._entries if x["id"] == eid), None)

    def open_browser(self, eid):
        # 1 - ABRO EL NAVEGADOR EN EL PUERTO LOCAL DEL TUNNEL:
        e = self._find(eid)
        if not e:
            return
        scheme = "https" if e["https"] else "http"
        try:
            webbrowser.open(f"{scheme}://localhost:{e['local_port_real'] or e['local_port']}")
        except Exception:
            pass

    def _on_output(self, eid):
        # 1 - LEO LA SALIDA DE kubectl: de ahi saco el puerto local real
        #     (cuando es Random) y detecto si el tunel quedo activo:
        e = self._find(eid)
        if not e or not e["process"]:
            return
        try:
            data = bytes(e["process"].readAllStandardOutput()).decode("utf-8", "replace")
        except Exception:
            return
        e["detalle"] = data.strip()[-300:]
        # 1a - kubectl imprime "Forwarding from 127.0.0.1:PORT -> PODPORT"
        #      (o "[::1]:PORT" en IPv6); capturo el puerto local real:
        m = re.search(r"(?:127\.0\.0\.1|\[::1\]):(\d+)", data)
        if m:
            e["local_port_real"] = int(m.group(1))
            if e["state"] != "activa":
                e["state"] = "activa"
                self.updated.emit()
            # 1a - ABRIR EN NAVEGADOR (una sola vez por arranque del tunel):
            if e.get("open_browser") and not e.get("browser_opened"):
                e["browser_opened"] = True
                try:
                    scheme = "https" if e["https"] else "http"
                    webbrowser.open(f"{scheme}://localhost:{e['local_port_real']}")
                except Exception:
                    pass

    def _on_exit(self, eid):
        e = self._find(eid)
        if not e:
            return
        if e["enabled"]:
            e["state"] = "caida"
            e["next_retry"] = time.time() + self.REINTENTO_SEG
        else:
            e["state"] = "detenida"
        self.updated.emit()

    def stop(self, eid):
        # 1 - APAGO EL TUNEL (deja de reintentar):
        e = self._find(eid)
        if not e:
            return
        e["enabled"] = False
    ...
    def resume(self, eid):
        # 1 - REANUDO UN TUNEL QUE EL USUARIO HABIA APAGADO:
        e = self._find(eid)
        if not e or e["enabled"]:
            return
        e["enabled"] = True
        self._spawn(e)
        self.updated.emit()
    ...
    def _monitor(self):
        # 1 - CADA 1 SEGUNDO: si un tunel habilitado se cayo, reintento
        #     cada 60 segundos hasta que el usuario lo apague:
        now = time.time()
        cambio = False
        for e in self._entries:
            if not e["enabled"]:
                continue
            proc = e.get("process")
            vivo = proc is not None and proc.state() != QProcess.NotRunning
            if not vivo and e["state"] in ("activa", "conectando"):
                e["state"] = "caida"
                e["next_retry"] = now + self.REINTENTO_SEG
                cambio = True
            if e["state"] == "caida" and now >= e.get("next_retry", 0):
                e["restarts"] += 1
                self._spawn(e)
                cambio = True
        if cambio:
    ...
    def shutdown(self):
        # 1 - AL CERRAR LA APP: MATO TODOS LOS kubectl port-forward:
        for e in self._entries:
            e["enabled"] = False
            try:
                if e["process"]:
...
class PortForwardPanel(QDialog):
    """Panel de conexiones abiertas: lista los port forwards con su pod,
    puertos y estado; permite detener/reanudar cada uno y abrir el navegador."""

    def __init__(self, manager, parent=None, new_forward_cb=None):
        super().__init__(parent)
        ...
        # 1 - FILA SUPERIOR: BOTON PARA FORWARDEAR UN PUERTO NUEVO:
        top = QHBoxLayout()
        top.addStretch()
        btn_new = QPushButton(f"{ICON_PLUS}  Forward nuevo...")
        btn_new.setToolTip("Abrir un tunnel port-forward a un pod")
        btn_new.clicked.connect(self._new_forward)
        top.addWidget(btn_new)
        lay.addLayout(top)

        self.table = QTableWidget(0, 7)
        self.table.setHorizontalHeaderLabels(
            ["#", "POD", "NAMESPACE", "PUERTO LOCAL", "PUERTO REMOTO", "ESTADO", "ACCIONES"]
        )
        ...
        # 1 - REFRESCO AUTOMATICO CADA 1 SEGUNDO (estado + cuenta regresiva)
        #     Y TAMBIEN CUANDO EL MANAGER AVISA QUE HUBO CAMBIOS:
        #     (antes estaba dentro de _new_forward, asi que abrir el panel
        #      sin clic en "Forward nuevo..." mostraba la lista vacia)
        self._timer = QTimer(self)
        self._timer.setInterval(1000)
        self._timer.timeout.connect(self._refresh)
        self._timer.start()
        self._manager.updated.connect(self._refresh)
        self._refresh()

    def _new_forward(self):
        # 1 - ABRO EL DIALOGO PARA FORWARDEAR UN PUERTO NUEVO:
        if self._new_cb:
            self._new_cb()
    ...
    def _refresh(self):
        ...
        for i, e in enumerate(entries):
            # 1 - DATOS BASICOS:
            local_port = e.get("local_port_real") or e["local_port"] or "-"
            vals = [
                str(e["id"]),
                e["pod_name"],
                e["namespace"],
            ...
            # 1a - ESTADO CON COLOR:
            if not e["enabled"]:
                estado, color = "Detenida", "#888888"
            elif e["state"] == "activa":
                estado, color = "Activa", "#4caf50"
            elif e["state"] == "conectando":
                estado, color = "Conectando...", "#f5c518"
            else:
                falta = max(0, int(e.get("next_retry", 0) - now))
                estado, color = f"Caida - reintento en {falta}s", "#ff9800"
            st_item = QTableWidgetItem(estado)
            st_item.setForeground(QBrush(QColor(color)))
            self.table.setItem(i, 5, st_item)

            # 1b - ACCIONES: Abrir navegador + Detener/Reanudar:
            w = QWidget()
            h = QHBoxLayout(w)
            h.setContentsMargins(2, 0, 2, 0)
            btn_open = QPushButton("Abrir")
            btn_open.setToolTip("Abrir en el navegador")
            btn_open.clicked.connect(lambda checked, eid=e["id"]: self._manager.open_browser(eid))
            h.addWidget(btn_open)
            if e["enabled"]:
                btn_stop = QPushButton("Detener")
                btn_stop.clicked.connect(lambda checked, eid=e["id"]: self._manager.stop(eid))
                h.addWidget(btn_stop)
            else:
                btn_resume = QPushButton("Reanudar")
                btn_resume.clicked.connect(lambda checked, eid=e["id"]: self._manager.resume(eid))
...
class PortForwardDialog(QDialog):
    """Dialogo para iniciar un port forward (estilo OpenLens).
    Si se le pasa mas de un pod, muestra un combo para elegir el pod.
    Con fixed_port no muestra combo de puerto (viene del badge clickeado)."""

    def __init__(self, pods, parent=None, fixed_port=None):
        super().__init__(parent)
        self._fixed_port = fixed_port
        self._pods = pods
        titulo = pods[0].metadata.name if len(pods) == 1 else "nueva conexion"
        self.setWindowTitle(f"{ICON_PORT}  Port Forwarding: {titulo}")
        self.resize(440, 230)

        lay = QVBoxLayout(self)
        lay.addWidget(QLabel("Elegi el pod y el puerto a forwardear:"))

        form = QFormLayout()
        self.combo_pod = QComboBox()
        for p in sorted(pods, key=lambda x: x.metadata.name):
            self.combo_pod.addItem(p.metadata.name, p)
        if len(pods) == 1:
            self.combo_pod.setVisible(False)
            form.addRow("Pod:", QLabel(f"{ICON_POD}  {pods[0].metadata.name}"))
        else:
            form.addRow("Pod:", self.combo_pod)

        if fixed_port:
            form.addRow("Puerto del pod:", QLabel(f"{ICON_PORT}  {fixed_port}"))
        else:
            self.combo_port = QComboBox()
            form.addRow("Puerto del pod:", self.combo_port)

        self.txt_local = QLineEdit()
        self.txt_local.setPlaceholderText("Random")
        form.addRow("Puerto local:", self.txt_local)

        self.chk_https = QCheckBox("https")
        self.chk_browser = QCheckBox("Abrir en navegador")
        ...
        # 1 - CARGO LOS PUERTOS DEL POD SELECCIONADO:
        if not fixed_port:
            self.combo_pod.currentIndexChanged.connect(self._reload_ports)
            self._reload_ports()

    def _reload_ports(self):
        # 1 - REPUEBO LOS PUERTOS SEGUN EL POD ELEGIDO:
        self.combo_port.clear()
        pod = self.combo_pod.currentData()
        if not pod:
            return
        vistos = set()
        for c in (pod.spec.containers or []):
            for p in (c.ports or []):
                if p.container_port not in vistos:
                    vistos.add(p.container_port)
                    self.combo_port.addItem(str(p.container_port))
        if self.combo_port.count() == 0:
            self.combo_port.addItem("80")

    def values(self):
        # 1 - DEVUELVO (pod, puerto_pod, puerto_local, https, abrir_navegador):
        local_txt = self.txt_local.text().strip()
        local = int(local_txt) if local_txt.isdigit() else None
        pod = self.combo_pod.currentData()
        puerto = self._fixed_port if self._fixed_port else int(self.combo_port.currentText())
        return (
            pod,
            int(puerto),
            local,
...
class PodDetailWindow(QDialog):
    ...
    def __init__(self, pod, parent=None, context_name=None, cluster_server=None, pf_starter=None):
        super().__init__(parent)
        self._pf_starter = pf_starter
    ...
    def _open_port_forward(self, pod, ports):
        # 1 - ABRO EL DIALOGO DE PORT FORWARD (estilo OpenLens) Y ARRANCO EL TUNNEL:
        if not self._pf_starter:
            QMessageBox.warning(
                self,
                f"{ICON_WARN} Port Forward",
                "No hay conexion activa: conectate a un cluster primero.",
            )
            return
        dlg = PortForwardDialog([pod], self)
        if dlg.exec() != QDialog.Accepted:
            return
        pod, pod_port, local_port, https, open_browser = dlg.values()
        try:
            self._pf_starter(pod, pod_port, local_port, https, open_browser)
        except Exception as e:
            QMessageBox.critical(self, f"{ICON_WARN} Port Forward", f"No se pudo iniciar el tunnel:\n{e}")
            return
        QMessageBox.information(
            self,
            f"{ICON_OK} Port Forward iniciado",
            "El tunnel quedo corriendo en background.\n"
            "Velo en el panel 'Conexiones abiertas' de la ventana principal\n"
            "(si se cae, se reconecta solo cada 1 minuto).",
        )
    ...
    def _add_containers(self, layout, pod):
        ...
        for c in spec.containers:
            ...
            # 5 - PUERTOS (+ boton Forward como OpenLens):
            if c.ports:
                ports_str = ", ".join(f"{p.container_port}/{p.protocol}" + (f" (name: {p.name})" if p.name else "") for p in c.ports)
                row_w = QWidget()
                row_l = QHBoxLayout(row_w)
                row_l.setContentsMargins(0, 0, 0, 0)
                row_l.addWidget(QLabel(ports_str))
                row_l.addStretch()
                btn_fwd = QPushButton(f"{ICON_PORT}  Forward...")
                btn_fwd.setToolTip("Abrir un tunnel port-forward a este pod")
                btn_fwd.clicked.connect(
                    lambda checked, pod_obj=pod, port_list=[p.container_port for p in c.ports]: self._open_port_forward(pod_obj, port_list)
                )
                row_l.addWidget(btn_fwd)
                form.addRow("Ports:", row_w)
            else:
